...
import { ArrowRight, FileText, Sparkles, Calendar, Tag } from "lucide-react";
...
export default function Blog() {
  ...
  const postsQuery = useQuery({
    queryKey: ["blog-published-posts"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("blog_posts")
        .select("id, title, slug, cover_image, summary, category, published_at, author_name")
        .eq("status", "published")
          ...
          {posts.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {posts.map((post: any) => (
                <article
                  key={post.id}
                  className="rounded-2xl border border-gray-100 bg-white overflow-hidden hover:shadow-lg transition-shadow cursor-pointer group"
                ...
                >
                  ...
                  <div className="p-6 space-y-3">
                    ...
                    <span className="inline-block text-sm font-medium text-blue-600 mt-2">
                      Ler artigo →
                    </span>
                  </div>
                </article>
