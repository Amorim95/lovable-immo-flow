...
WITH CHECK (
  auth.uid() = id 
  AND (users.role = (SELECT role FROM public.users WHERE id = auth.uid())) -- Prevent role changes by users themselves
);
...
USING (
  ...
  (EXISTS (
    SELECT 1 FROM public.users auth_user 
    WHERE auth_user.id = auth.uid() 
    AND auth_user.role = 'gestor' 
    AND auth_user.status = 'ativo'
    AND auth_user.equipe_id = users.equipe_id
    AND auth_user.equipe_id IS NOT NULL
  ))
);
...
-- Security Fix 4: Strengthen password validation function
